...
decode(Bin) ->
    try json:decode(Bin) of
        #{~"type" := ~"message"} = Data ->
            decode_event(Data);
        #{~"seq_id" := SeqId} = Data when is_integer(SeqId) andalso SeqId > 0 ->
            decode_reqresp(Data);
        _ ->
            error
    catch
...
-spec decode_event(json:decode_value()) -> {ok, event()} | error.
decode_event(#{
    ~"type" := ~"message",
    ~"author" := Author,
    ~"text" := Text
}) when is_binary(Author) and is_binary(Text) ->
    {ok, #message{author = Author, text = Text}};
decode_event(_) ->
    error.

-spec decode_reqresp(json:decode_value()) -> {ok, req() | resp()} | error.
...
decode_reqresp(#{~"seq_id" := SeqId, ~"type" := ~"message_sent"}) ->
...
decode_reqresp(#{~"seq_id" := SeqId, ~"type" := ~"auth_ok"}) ->
    {ok, #auth_ok{seq_id = SeqId}};
decode_reqresp(#{~"seq_id" := SeqId, ~"type" := ~"auth_error"}) ->
    {ok, #auth_error{seq_id = SeqId}};
decode_reqresp(#{~"seq_id" := SeqId, ~"type" := ~"already_connected"}) ->
    {ok, #already_connected{seq_id = SeqId}};
decode_reqresp(_Data) ->
    error.
